...
| Compute environment | Local or remote software profile used to execute a calculation. Query it with `ts_environment` or `/compute`. |
| Claim relation | Directed relation between Claims, such as support, conflict, or dependency. |
| ChangeSet | Explicit list of map operations submitted through `ts_change`. The Kernel validates and commits it as one revision. |
...
| Platform | Local or remote environment in which a backend runs. |
...
Root verifies and records the relevant fact or issue through `ts_change`.
