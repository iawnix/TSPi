...
```text
ts_state         有边界地读取 ResearchMap 与计算状态
ts_change        提交一个原子 ResearchMap ChangeSet
ts_environment   本地与远端计算环境目录
ts_calc          一个有边界的计算生命周期
ts_dispatch      暂停或恢复一个 Node 的新任务派发
ts_analyze       已注册的本地分析
ts_seed/import   Node 所属的结构与输入 Artifact
ts_compare       确定性结构比较
ts_render        已注册的可视化 Artifact
ts_report        绑定 revision 的报告包
ts_review/reply  建议性 Review 与 Root 处置
ts_notify        已配置的投递事件
```
...
或 `ts_state mode=summary`。research extension 可以在 prompt 中加入有边界的摘要，但不能
写入 map。方法选择、结果解释与停止决策仍由 Root Agent 负责。

`ts_state` 模式包括 `map`、`summary`、`detail`、`locate`、`validate`、
...
Artifact 后，通过 `ts_change` 应用科学解释。
