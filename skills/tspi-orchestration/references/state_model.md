...
this same object. Clients do not maintain a second research-state model.

## Objects

| Object | Role | Important fields |
| --- | --- | --- |
| `ResearchPhase` | groups related Nodes | `title`, `objective`, `node_ids` |
...
acyclic. A NodeGate must have a passing latest evaluation before a Node can be
closed as `completed`.
...
`detail`, `locate`, `operations`) plus compute modes (`artifacts`,
