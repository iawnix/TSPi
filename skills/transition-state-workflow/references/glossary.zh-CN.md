# Public Glossary / 公开术语表

[English](glossary.md) | [简体中文](glossary.zh-CN.md)

这些名称用于记录和责任划分。科学问题、方法、标签、关系标签和验证维度
保持开放；名称本身不能授权一个行动。
...
| ResearchPhase | 用于人类导航的阶段分组，不控制 Node 生命周期或研究顺序。 |
| ResearchNode (Node) | 一个有边界的问题或决策事件，包含依赖、Claim 范围和结果。 |
| Claim | 带假设、反证条件、状态和引用记录的科学陈述。 |
| ClaimRelation | Claim 之间已记录的科学关系，不具有执行权限。 |
| Artifact | 带标识、摘要和来源的文件；输出文件本身不是科学结论。 |
...
| Compute | 执行固定操作计划的子模型，不拥有科学决策权限。 |
| Review | 对一个 Claim 的独立、有边界的建议性评估；Root Agent 通过 `ts_reply` 记录处置。 |
| Reviewer role | Review 中版本化的专业角色和预算；当前角色是 `general`，权限仍是建议性。 |

工具名和字段名必须以实时合约为准。`frontier` 和 `delta` 是上下文读取模式；
`$alias` 只在一个 Decision 草稿中有效。修订和摘要用于绑定记录，不用于评价
科学质量或选择下一步研究行动。
