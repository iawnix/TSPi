...
session. Structure seed, input import, Compute, Render, Report, remote inspection, workspace
...
Review task and bound inputs are durable; normal terminal handling writes its
actions, result, and run summary. A process crash can leave a pending/unknown
journal, and no background replay exists.
...
`TS Activity` presents current Review and deterministic-tool activity without
gaining authority. It is transient. `/ts-subagent-history` is a paginated
read-only Review-run browser. Pi's normal expand key expands/collapses tool
details globally; the history browser paginates its own list.
