...
The remote `ase_neb` software profile must select a Python environment that can
import ASE, NumPy, and the matching `ts_agent.backends.ase_neb_runner`. Set
`TS_ASE_NEB_XTB` in the profile environment when `xtb` is not on `PATH`. Run
`ts_remote doctor` before submitting.
