...
ASE 负责 NEB 插值和路径优化。默认的 xTB CLI calculator 为每个 image 提供
能量和笛卡尔梯度；将 `calculator = "gaussian_cli"` 设置为 Gaussian 模式后，
每个 image 会执行配置的 Gaussian `Force` 任务并解析梯度。Gaussian 模式需要
`gaussian_route`、`gaussian_multiplicity`、`gaussian_nproc` 和 `gaussian_mem`。
支持的 xTB 电子结构方法为 `gfn1` 和 `gfn2`，不会隐式替换 calculator。托管 runner 对 image 串行计算且不启用 ASE 预条件器；并行 image
...
`charge`、`uhf`、`accuracy`、`electronic_temperature`、`solvent_model`、
`solvent`、`gaussian_route`、`gaussian_multiplicity`、`gaussian_nproc` 和
`gaussian_mem`。
...
`ts_agent.backends.ase_neb_runner`。`calculator` 默认是 `xtb_cli`；xTB 模式在 Backend binding 的环境变量中将
...
导入失败、runner 版本不一致或所选 calculator 的程序探测失败都属于运维失败，不能作为反应路径
证据。不得从计算作业中安装包，也不得把软件安装到研究工作区。按照
按照[远端计算配置](../../../docs/INSTALLATION.zh-CN.md)操作，并且只有登录节点和计算节点
冒烟检查都通过后才能切换 Backend binding。
