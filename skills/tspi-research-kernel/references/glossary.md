...
| Compute environment | Named local or remote execution environment with bound Backends. Query it with `compute.environment` or `/compute`. |
| Claim relation | Directed relation between Claims, such as support, conflict, or dependency. |
| ChangeSet | Explicit list of map operations submitted through `research.change`. The Kernel validates and commits it as one revision. |
...
Root verifies and records the relevant fact or issue through `research.change`.
