...
使用 `ts_report` 时加载本 Skill。map 与 Artifact 合同使用 `tspi-orchestration`，需要图像时
加载 `tspi-render`。

报告必须来自有效工作区和已注册逻辑 Artifact。分开展示当前 ResearchMap revision、
Phase、Claim、Node 状态和结果、FactFinding、IssueFinding、Gate criteria/evaluations、
未决问题及计算历史。不要把运行成功写成 Claim 结论；每个数值或结构陈述都引用相关
Finding 和来源 Artifact。
