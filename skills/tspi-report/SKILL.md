...
Use this Skill for `report.build`. Use `tspi-research-kernel` for ResearchMap
...
Before an export, use `research.read` to read the current map and record any changed
Node or Claim status with `research.change`. Create a new report package per export;
