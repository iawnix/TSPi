...
def create_calculation_intent(root: str | Path, request: dict[str, Any]) -> dict[str, Any]:
    ...
    input_refs = _resolve_request_input_refs(
        workspace,
        node_id,
        backend,
        task_type,
...
def _resolve_request_input_refs(
    workspace: Path,
    node_id: str,
    backend: str,
    task_type: str,
    supplied_refs: dict[str, str],
) -> dict[str, str]:
    required, _ = BACKENDS[backend][task_type]
    unexpected = sorted(set(supplied_refs) - required)
    if unexpected:
        raise ComputeContractError(
            f"{backend} input roles must be exactly {sorted(required)}; unexpected={unexpected}"
        )
    resolved: dict[str, str] = {}
    for role in sorted(required):
        supplied = supplied_refs.get(role)
        if supplied is not None:
            candidate = supplied if "/" in supplied else f"nodes/{node_id}/inputs/{supplied}"
            normalized = _workspace_ref(workspace, candidate, read=True)
            _require_compute_input_ref(normalized)
            resolved[role] = normalized
            continue
        resolved[role] = _auto_resolve_node_input(workspace, node_id, role, required)
    if len(set(resolved.values())) != len(resolved):
        raise ComputeContractError("distinct calculation input roles must resolve to distinct artifacts")
    return resolved


def _auto_resolve_node_input(
    workspace: Path,
    node_id: str,
    role: str,
    required_roles: set[str],
) -> str:
    suffixes = AUTO_INPUT_SUFFIXES.get(role)
    shared_suffix_roles = [
        candidate
        for candidate in required_roles
        if candidate != role and AUTO_INPUT_SUFFIXES.get(candidate) == suffixes
    ]
    if suffixes is None or shared_suffix_roles:
        raise ComputeContractError(
            f"input_refs.{role} is required because that role cannot be inferred unambiguously"
        )
    input_dir = workspace / "nodes" / node_id / "inputs"
    candidates = sorted(
        path
    ...
    if len(candidates) != 1:
        ...
        raise ComputeContractError(
            f"input_refs.{role} requires exactly one matching node input; found {len(candidates)}: {names}"
        )
...
def _raw_prepared_task(workspace: Path, intent: dict[str, Any]) -> PreparedTask:
    inputs = {}
