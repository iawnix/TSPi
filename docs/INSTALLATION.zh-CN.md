...
不会停止 worker 或当前 turn。Native Pi Harness 是唯一支持的后端。

规范 format-4 transcript 位于 `.pi/app-server-host/sessions/<encoded-cwd>/`，请求回执、scheduler
lease、Monitor outbox 也位于安装级 Host state。workspace `.pi/sessions` 历史不被 Native
Harness 接受。systemd 重启后这些 durable operation/queue ID 仍可恢复，客户端通过新的 Host
epoch/cursor 重新附着。
