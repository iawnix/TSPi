...
证据核验后记录为 `FactFinding` 或 `IssueFinding`。Gate 评估结果、开放 Finding 和
回溯决定进入下一步研究。TS Web 直接渲染 canonical ResearchMap，终端与 TS Phone
...
研究任务，ClaimGate 评估累积 Finding，ResearchMap 是客户端直接渲染的规范状态。
