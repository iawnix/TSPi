...
> **Figure X. Abstract architecture of the TSPi transition-state research workflow.** The Root Agent proposes scientific questions, hypotheses, methods, and explicit ChangeSets. A deterministic Research Kernel is the single mutation authority for the Phase–Node–Claim graph and its Finding and Gate records. Bounded capability adapters produce artifacts and transient candidates; only verified evidence is recorded as a FactFinding or IssueFinding. Gate evaluations and open findings feed the next research step. TS Web renders the canonical ResearchMap, while Phone/terminal clients connect to the shared Pi session and invoke the Agent and tools; neither becomes a second scientific state store.

The artwork uses a restrained color-blind-aware palette: teal for canonical state, navy for data flow, amber for bounded capabilities, violet for provenance, coral for validation and decision feedback, and blue for the evidence plane. TS Web is a read-only canonical ResearchMap browser; Phone and terminal are equal interactive Pi clients and may prompt the shared App Server, invoke tools, and receive tool results. It is intentionally abstract and uses short labels so it can serve as a main-text architecture figure rather than a software flowchart. The source viewBox is 1200 × 720; main section labels remain 14 units or larger, with compact registry labels reduced only where needed to stay inside their cards.
...
> **Figure X. TSPi framework for auditable reaction-mechanism research.** A Root Agent formulates a Claim together with falsifiers and a stopping rule. The Research Kernel creates bounded Research Nodes, records provenance, and is the sole mutation boundary. Skills and plugins execute geometry generation, transition-state optimization, frequency/IRC checks, and path comparisons, producing inspectable artifacts rather than scientific conclusions. After Root verification, artifacts become FactFindings or IssueFindings in the map. A NodeGate closes an individual research task, whereas a ClaimGate evaluates whether accumulated Findings support, contradict, or leave the Claim open. The resulting interpretation determines the next dependent Node, branch, or backtracking step; the ResearchMap is the canonical state rendered by clients.
