...
def test_energy_profile_keeps_stoichiometric_pool_and_renderer_contract():
    ...
    schema = json.loads((Path(__file__).resolve().parents[1] / "contracts/ts-render/curve-data.schema.json").read_text())
...
def test_report_and_web_link_analyses_to_node_and_observation(tmp_path):
    from tests.workspace_helpers import apply_change
