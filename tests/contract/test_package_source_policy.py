...
process.stdout.write(JSON.stringify({{
  ...
  tests: decision("read", {json.dumps(str(ROOT / 'tests' / 'integration' / 'test_pi_runtime_integration.py'))}),
  extension: decision("ls", {json.dumps(str(ROOT / 'extensions'))}),
  symlink: decision("read", {json.dumps(str(implementation_link / 'integration' / 'test_pi_compute_tools.py'))}),
...
const blocked = await handlers.tool_call({{
  ...
  toolCallId: "read-1",
  toolName: "read",
  input: {{ path: {json.dumps(str(ROOT / 'tests' / 'integration' / 'test_pi_runtime_integration.py'))} }},
}}, ctx);
