...
if (!provider || !modelId || !outputPath) throw new Error("Usage: node tests/mechanism_live_eval.mjs PROVIDER MODEL REPEATS OUTPUT.json");
...
while (pending.length) {
  ...
  const fixture = JSON.parse(execFileSync(process.env.TS_AGENT_PYTHON || "python3", ["-m", "tests.mechanism_eval_fixture", join(scratch, `${caseId}-${repetition}`), caseId], { cwd: repository, encoding: "utf8" }));
