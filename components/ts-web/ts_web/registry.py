...
def path_has_symlink(path: Path) -> bool:
    ...
    for component in (Path(current.anchor), *current.relative_to(current.anchor).parts):
        current_component = Path(current.anchor) / component if component != Path(current.anchor) else component
...
def read_json(path: Path) -> object:
    if path_has_symlink(path) or not path.is_file() or path.is_symlink():
        raise ValueError(f"registry file is not a regular file: {path}")
...
def write_json(path: Path, data: object) -> None:
    ...
    if path_has_symlink(path):
        raise ValueError(f"registry path contains a symbolic link: {path}")
