// Legacy experimental entrypoint, retained only for explicit migration/debugging.
...
// is a thin Pi client: keep Pi's own presentation, command registry, and input
// loop intact. Presentation packages are loaded only when the caller requests
// them explicitly; the wrapper must not add a presentation package.
const piForwarded = forwarded;
...
function startMonitorWorker(wrapper, forwarded, env, workspaceRoot, packageRoot) {
  ...
  // The legacy experimental server has no Host identity unless its caller
  // supplied the managed directory and server id. The current monitor worker
  // requires that endpoint; skip it for unbound legacy sessions instead of
  // turning a debug-only Pi server into a restart loop.
  const serverId = forwardedOption(forwarded, "--server-id");
  if (!wrapper.directory || !serverId) return undefined;
  const args = [
    join(packageRoot, "apps/app-server/pi-monitor-worker.mjs"),
    "--workspace-root", workspaceRoot,
    "--host-socket", join(resolve(wrapper.directory), `${serverId}.sock`),
  ];
