...
const CAPABILITY_KINDS = ["compute", "proof"];
  ...
  return {
    ...
    async execute(_toolCallId, params, _onUpdate, toolContext, _invocation, context) {
      ...
      if (mode === "artifacts") {
        ...
        if (!params.capabilityKind) throw new Error("state mode=capabilities requires capabilityKind=compute or proof");
...
function requireNativeWrites(toolName) {
  if (process.env.TSPI_NATIVE_WRITES !== "1") {
    throw new Error(`${toolName} is disabled for native Pi sessions; restart with --allow-writes after acquiring the workspace guard`);
  }
}
