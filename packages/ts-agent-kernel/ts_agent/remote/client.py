"""Thin OpenSSH/SCP client used only by ts_remote."""
...
class SSHClient:
    def __init__(self, profile: RemoteProfile, *, runner: Runner = subprocess.run) -> None:
    ...
    def run(
        ...
        *,
        ...
        argv = [*self._ssh_prefix(), self.profile.ssh_host, remote_command]
    ...
    def upload(self, source: Path, remote_path: str) -> CommandResult:
        ...
        argv = [*self._scp_prefix(), str(source), f"{self.profile.ssh_host}:{remote_path}"]
    ...
    def download(self, remote_path: str, destination: Path) -> CommandResult:
        ...
        argv = [*self._scp_prefix(), f"{self.profile.ssh_host}:{remote_path}", str(destination)]
