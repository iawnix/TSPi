"""Shared read/write helpers for the persistent explorer workspace registry.

The registry is a small JSON file listing every TS-search workspace the
long-lived explorer service should expose. Workspace creation registers new
entries here; the running service reloads the file on change, so no restart is
needed when a new search task starts.
"""
...
def register_workspace(
    ...
    *,
    ...
    Entries are keyed by id; re-registering the same workspace updates the
