...
Facts are optional and must use the matching kind (`render` or `report`). Cite only task allowlist refs or artifacts returned by the typed tool. Never include credentials, hypothesis status, branch context, accepted-TS/pathway fields, strict pathway decisions, study completion, or next-step decisions in the result.
