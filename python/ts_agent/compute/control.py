...
def _validate_intent_node_scope(workspace: Path, intent: dict[str, Any], node: dict[str, Any]) -> None:
    if node.get("schema_version") != "ts-research-node/1":
        raise ComputeContractError("calculations require a ts-research-node/1 owner")
    if node.get("status") != "open":
    ...
    try:
        _load_node(workspace, str(source_node))
